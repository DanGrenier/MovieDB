module Api
  module V1
    class MovieScoresController < ApplicationController
      before_action :not_for_admin, only: [:create]
      before_action :load_movie, only: [:create]
      before_action :load_score, only: [:destroy]
      before_action :only_delete_your_score, only:[:destroy]
      ...
      def only_delete_your_score
        ...
        raise AuthorizationError unless (@score.user == current_user || current_user.admin)
      end   
    end
  end
end